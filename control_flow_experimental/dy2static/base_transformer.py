...
class ForLoopTuplePreTransformer(BaseTransformer):
    ...
    >>> UUID_iterator = dy2s.Indexable(B)  # make iterator-only to indexable list.
    >>> for UUID_target in UUID_iterator:
    >>>     A = dy2s.Unpack(UUID_target, structure)
    >>>     C
...
class ForNodeVisitor:
    ...
    def _build_var_len_assign_node(self):
        ...
        convert_len_node_source_str = '{} = len({})'.format(
