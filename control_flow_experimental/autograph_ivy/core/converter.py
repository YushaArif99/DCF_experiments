...
"""
import enum
from control_flow_experimental.autograph_ivy.pyct import parser, templates, transformer
...
class ConversionOptions(object):
    ...
    def as_tuple(self):
        return (
           self.recursive, self.user_requested, self.internal_convert_user_code, self.optional_features
        )
    ...
    def to_ast(self):
        ...
        if self == STANDARD_OPTIONS:
            return parser.parse_expression('ivy__.STD')

        template = """
        ivy__.ConversionOptions(
        ...
        def list_of_features(values):
            return parser.parse_expression('({})'.format(', '.join(
                'ivy__.{}'.format(str(v)) for v in values)))
