...
BaseArgumentTypes = Union[str, int, float, bool, complex]
    ...
    def __init__(self, graph: 'Graph', name: str, op: str, target: 'Target',
        ...
        if op == 'call_function':
            if not callable(target):
                raise ValueError(f'Node [graph = {graph}, name = \'{name}\'] target {target} has type {type(target)} '
...
def map_arg(a: Argument, fn: Callable[[Node], Argument]) -> Argument:
    ...
    """
    assert callable(fn), "fx.map_arg(a, fn): fn must be a callable"
